        ...
        tabItem(tabName = "About",
                fluidRow(
                    box(width = 12,
                        ...
                        "The data set is updated roughly once a day at varying times, and in order to minimize bugs and errors caused by missing values, the data in Maps and Stats are displayed with a 1 day delay, as indicated when hovering on the graphs.",
...
server <- function(input, output) {
    ...
    output$variation_chart <- renderPlotly({
        trend %>%
            ...
            add_bars(x = ~Canton,
                     y= ~change_percemt,
                     color = if(trend$change_percemt < 0){I("mediumpurple2")}else{I("orange1")},
                     opacity = 0.6,
    ...
    output$cases <- renderPlotly({
        data_swiss %>%
            ...
            add_bars(x = ~reorder(Canton, -ncumul_conf_per_10thous),
                     ...
                     opacity = 0.5,
                     hoverinfo = "text",
                     text = ~paste(date, "<br>", Canton, "<br>", round(ncumul_conf_per_10thous, 0), "cases every 10000 people")) %>%
            layout(yaxis = list(title = "Total Covid-19 cases", showgrid = F, fixedrange = TRUE),
                   xaxis = list(title = " ", fixedrange = TRUE)) %>%
            config(displayModeBar = FALSE, displaylogo = FALSE)
    })
    ...
    output$deaths <- renderPlotly({
        data_swiss %>%
            ...
            add_bars(x = ~reorder(Canton, -ncumul_deceased_per_10thous),
                     ...
                     color = I("black"),
                     opacity = 0.5,
                     text = ~paste(date, "<br>", Canton, "<br>", round(ncumul_deceased_per_10thous, 0), "deaths every 10000 people")) %>%
            layout(yaxis = list(title = "Total deaths", showgrid = F, fixedrange = TRUE),
                   xaxis = list(title = " ", fixedrange = TRUE)) %>%
            config(displayModeBar = FALSE, displaylogo = FALSE)
    })
