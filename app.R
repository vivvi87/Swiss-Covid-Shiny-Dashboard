...
data_swiss <- data_swiss %>%
    ...
    fill(ncumul_conf, ncumul_deceased, ncumul_conf_per_10thous, ncumul_deceased_per_10thous)
...
trend <- trend %>%
    mutate(change_percemt = round((tot14days_last-tot14days_previous)/tot14days_previous*100, 0))
    ...
    tabItems(
                ...
                fluidRow(
                    ...
                    box(width = 2,
                        sliderInput("dates", "Select dates:",
                                    ...
                                    value = c(as.Date("2020-10-10"), max(data_total_swiss$date))
                        )
                    )
                ),
                fluidRow(
                    ...
                    box(width = 2,
                        sliderInput("dates_canton", "Select dates:",
                                    ...
                                    value = c(as.Date("2020-10-10"), max(data_swiss$date))
                        ),
        ...
        tabItem(tabName = "About",
                fluidRow(
                    box(width = 12,
                        h2("About"),
                        "This dashboard has been built using the data found in the GitHub repository ", a("https://github.com/openZH/covid_19", href = "https://github.com/openZH/covid_19"), ", which collect Covid-19 data for Switzerland and Lichtenstein.",
                        "The data set is updated roughly once a day at varying times, and in order to minimize bugs and errors caused by missing values, the data in Maps and Stats are displayed with a 3 days delay, as indicated when hovering on the graphs.",
                        "The data analysis as well as the source code of the dashboard can be found at ", a("https://github.com/vivvi87/Swiss-Covid-Shiny-Dashboard", href = "https://github.com/vivvi87/Swiss-Covid-Shiny-Dashboard"), ". Both source code and data can be directly accessed from the sidebar."
                    )
                )
        ),
...
#            mutate(Canton = as.character(Canton)) %>%
    ...
    output$variation_map <- renderLeaflet({
        rampcols <- if(length(which(trend_swiss_geo$change_percemt > 0)) == 0){colorRampPalette(colors = c("purple", "white"), space = "Lab")(length(which(trend_swiss_geo$change_percemt < 0)))}else if(length(which(trend_swiss_geo$change_percemt < 0)) == 0){colorRampPalette(colors = c("moccasin", "orange"), space = "Lab")(length(which(trend_swiss_geo$change_percemt > 0)))}else{c(colorRampPalette(colors = c("purple", "white"), space = "Lab")(length(which(trend_swiss_geo$change_percemt < 0))), colorRampPalette(colors = c("moccasin", "orange"), space = "Lab")(length(which(trend_swiss_geo$change_percemt > 0))))}
    ...
    output$variation_chart <- renderPlotly({
        trend %>%
            ...
            add_bars(x = ~Canton,
                     y= ~change_percemt,
                     color = I(ifelse(trend$change_percemt < 0, "#9966FF", "#FF9900")),
                     opacity = 0.6,
    ...
    output$map_cases <- renderLeaflet({
        ...
        labels3 <- sprintf(
            ...
            data_swiss_geo_last$date, data_swiss_geo_last$KTNAME, round(data_swiss_geo_last$ncumul_conf_per_10thous, 0)
        ) %>% lapply(htmltools::HTML)
        data_swiss_geo_last %>%
            leaflet(options = leafletOptions(minZoom = 7.2)) %>%
            setView(lat = 46.7, lng = 8.4, zoom = 7.2) %>%
    ...
    output$map_deaths <- renderLeaflet({
        ...
        labels5 <- sprintf(
            ...
            data_swiss_geo_last$date, data_swiss_geo_last$KTNAME, round(data_swiss_geo_last$ncumul_deceased_per_10thous, 0)
        ) %>% lapply(htmltools::HTML)
        data_swiss_geo_last %>%
            leaflet(options = leafletOptions(minZoom = 7.2)) %>%
            setView(lat = 46.7, lng = 8.4, zoom = 7.2) %>%
    ...
    output$cases_abs <- renderPlotly({
        data_swiss_geo %>%
            ...
            plot_ly() %>%
#            filter(date == max(date)-3) %>%
            add_bars(x = ~reorder(KTNAME, -ncumul_conf),
                     y = ~ncumul_conf,
                     color = I("black"),
                     opacity = 0.5,
                     hoverinfo = "text",
                     text = ~paste(date, "<br>", KTNAME, "<br>", ncumul_conf, "total cases")) %>%
            layout(yaxis = list(title = "Total Covid-19 cases", showgrid = F, fixedrange = TRUE),
    ...
    output$cases <- renderPlotly({
        data_swiss_geo %>%
            ...
            plot_ly() %>%
#            filter(date == max(date)-3) %>%
            add_bars(x = ~reorder(KTNAME, -ncumul_conf_per_10thous),
                     y = ~ncumul_conf_per_10thous,
                     color = I("black"),
                     opacity = 0.5,
                     hoverinfo = "text",
                     text = ~paste(date, "<br>", KTNAME, "<br>", round(ncumul_conf_per_10thous, 0), "cases every 10000 people")) %>%
            layout(yaxis = list(title = "Total Covid-19 cases", showgrid = F, fixedrange = TRUE),
                   xaxis = list(title = " ", fixedrange = TRUE)) %>%
            config(displayModeBar = FALSE, displaylogo = FALSE)
    })
    output$deaths_abs <- renderPlotly({
        data_swiss_geo %>%
            ...
            plot_ly() %>%
#            filter(date == max(date)-3) %>%
            add_bars(x = ~reorder(KTNAME, -ncumul_deceased),
                     y = ~ncumul_deceased,
                     color = I("black"),
                     opacity = 0.5,
                     hoverinfo = "text",
                     text = ~paste(date, "<br>", KTNAME, "<br>", ncumul_deceased, "total deaths")) %>%
            layout(yaxis = list(title = "Total deaths", showgrid = F, fixedrange = TRUE),
                   xaxis = list(title = " ", fixedrange = TRUE)) %>%
            config(displayModeBar = FALSE, displaylogo = FALSE)
    })
    output$deaths <- renderPlotly({
        data_swiss_geo %>%
            ...
            plot_ly() %>%
#            filter(date == max(date)-3) %>%
            add_bars(x = ~reorder(KTNAME, -ncumul_deceased_per_10thous),
                     y = ~ncumul_deceased_per_10thous,
                     hoverinfo = "text",
                     color = I("black"),
                     opacity = 0.5,
                     text = ~paste(date, "<br>", KTNAME, "<br>", round(ncumul_deceased_per_10thous, 0), "deaths every 10000 people")) %>%
            layout(yaxis = list(title = "Total deaths", showgrid = F, fixedrange = TRUE),
                   xaxis = list(title = " ", fixedrange = TRUE)) %>%
            config(displayModeBar = FALSE, displaylogo = FALSE)
    })
