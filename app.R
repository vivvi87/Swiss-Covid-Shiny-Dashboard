        ...
        tabItem(tabName = "About",
                fluidRow(
                    box(width = 12,
                        ...
                        "The data set is updated roughly once a day at varying times, and in order to minimize bugs and errors caused by missing values, the data in Maps and Stats are displayed with a 2 days delay, as indicated when hovering on the graphs.",
...
server <- function(input, output) {
    ...
    output$variation_map <- renderLeaflet({
        rampcols <- if(length(which(trend_swiss_geo$change_percemt > 0)) != 0){c(colorRampPalette(colors = c("purple", "white"), space = "Lab")(length(which(trend_swiss_geo$change_percemt < 0))), colorRampPalette(colors = c("moccasin", "orange"), space = "Lab")(length(which(trend_swiss_geo$change_percemt > 0))))}else{rc1 <- colorRampPalette(colors = c("purple", "white"), space = "Lab")(length(which(trend_swiss_geo$change_percemt < 0)))}
